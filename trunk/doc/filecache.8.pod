...
filecache - page cache tool for fast system boot/application startup
...
take a snapshot of I</proc/filecache> with pages satisfy the CONDITION.
CONDITION is a piece of ruby code that returns C<true>/C<false>.
avaiable fields:
...
delete files satisfying I<CONDITION>.
available fields:
...
=item B<{dstdir}/{devname}>

=item B<{dstdir}/{nfspath}>
...
To save a copy of the current cache contents:
   ...
   filecache --snapshot --dump /tmp/`date +'%F'`/
